...
export const FamilyGoalModal: React.FC<FamilyGoalModalProps> = ({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-70 flex items-center justify-center p-3 sm:p-5 md:p-6 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="bg-yellow-50 rounded-[2.5rem] p-5 sm:p-7 max-w-3xl w-full shadow-2xl border-4 border-yellow-300 max-h-[92vh] flex flex-col my-auto"
      >
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b-2 border-yellow-200/80 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-amber-400 border-2 border-amber-300 flex items-center justify-center text-2xl shadow-inner transform -rotate-3">
              🏆
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                {parentUnlocked ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[10px] font-black px-2 py-0.5 rounded-full bg-slate-200 text-slate-700">
          ...
          <button
            ...
            className="w-10 h-10 rounded-2xl bg-yellow-200/80 hover:bg-yellow-300 border-2 border-yellow-300 flex items-center justify-center text-slate-700 font-bold transition-all active:scale-95 cursor-pointer"
          >
        ...
        {/* Success Toast */}
        {successToast && (
          <div className="mt-3 p-3 rounded-2xl bg-emerald-500 text-white font-extrabold text-xs sm:text-sm flex items-center justify-between shadow-md animate-bounce">
            <div className="flex items-center gap-2">
        ...
        {/* Tabs Bar */}
        <div className="flex items-center gap-1.5 p-1.5 rounded-2xl bg-yellow-200/60 border border-yellow-300 mt-4 overflow-x-auto shrink-0">
          {[
            { id: 'current', label: 'Active Goal & Progress', icon: Target },
            { id: 'presets', label: 'Alternative Goal Ideas', icon: Compass },
            { id: 'saved', label: `Saved Library (${(database.savedFamilyGoals || []).length})`, icon: Trophy },
            { id: 'custom', label: 'Create Custom Goal', icon: Plus },
          ].map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-black whitespace-nowrap transition-all cursor-pointer ${
                  isActive
