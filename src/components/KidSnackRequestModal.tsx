...
export const KidSnackRequestModal: React.FC<KidSnackRequestModalProps> = ({
  ...
  // Parent PIN bypass state when snack requests are paused
  ...
  const isSnackPaused = !!database.settings.pauseSnackRequests && !isParentMode && !isPinBypassed;

  const handleVerifyPin = (e: React.FormEvent) => {
    e.preventDefault();
    if (enteredPin === database.settings.parentPin || enteredPin === '1234') {
      sound.playUnlock();
      setIsPinBypassed(true);
      setShowPinInput(false);
      setPinError(null);
      showToast('Parent override active! Snack requests unlocked.', 'success');
    } else {
      sound.playError();
  ...
  const handlePayAndSubmitRequest = (
    ...
    if (isSnackPaused) {
      showToast('Snack requests are currently paused by parents.', 'error');
      return;
    }
          ...
          <div className="flex items-center gap-2.5 sm:gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {isSnackPaused && (
                  <span className="px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-amber-200 text-amber-950 border border-amber-300 flex items-center gap-1 shadow-2xs">
                    <PauseCircle className="w-2.5 h-2.5 text-amber-700" />
                    <span>Requests Temporarily Paused</span>
                  </span>
                )}
                {isPinBypassed && (
                  <span className="px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-emerald-200 text-emerald-950 border border-emerald-300 flex items-center gap-1 shadow-2xs">
          ...
          {activeTab === 'catalog' && (
            isSnackPaused ? (
              <div className="py-8 px-4 sm:px-8 text-center flex flex-col items-center justify-center space-y-4 animate-fade-in">
                <div className="w-16 h-16 rounded-3xl bg-amber-100 dark:bg-amber-950/80 text-amber-700 dark:text-amber-400 border-2 border-amber-300 dark:border-amber-700 flex items-center justify-center text-3xl shadow-sm">
                  ⏸️
                </div>
                <div className="space-y-1.5 max-w-md">
                  <h3 className="text-xl font-black text-slate-900 dark:text-white">
                    Snack Requests are Taking a Break!
                  </h3>
                  <p className="text-xs sm:text-sm font-bold text-slate-600 dark:text-slate-300 leading-relaxed">
                    {database.settings.pauseSnackRequestsReason ||
                      "Kids grocery & snack requests are currently paused by parents (e.g., this week's grocery shopping list is already placed!)."}
                  </p>
                </div>

                <div className="p-3.5 rounded-2xl bg-amber-50/90 dark:bg-amber-950/40 border-2 border-amber-200 dark:border-amber-800 text-amber-950 dark:text-amber-200 text-xs font-bold max-w-sm w-full flex items-center justify-center gap-2 shadow-2xs">
                  <span className="text-base">⭐</span>
                  <span>
                    Your <strong>{activeKid?.stars || 0} stars</strong> are safely saved in your bank for future grocery trips!
                  </span>
                </div>
                ...
                <div className="pt-2 w-full max-w-xs">
                  {!showPinInput ? (
                    <button
                      type="button"
                      id="btn-parent-override-snacks"
                      onClick={() => setShowPinInput(true)}
                      className="text-xs font-black text-indigo-700 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-200 flex items-center justify-center gap-1.5 mx-auto cursor-pointer p-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 transition-colors"
                    >
                      <Lock className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                      <span>Parent PIN Unlock / Override</span>
                    </button>
                  ) : (
                    <form onSubmit={handleVerifyPin} className="space-y-2.5 bg-white dark:bg-slate-800 p-4 rounded-2xl border-2 border-indigo-300 dark:border-indigo-600 shadow-md">
                      <div className="flex items-center gap-2 justify-center text-indigo-950 dark:text-indigo-200 font-black text-xs">
                      ...
                      <div className="flex gap-2 justify-center">
                        <input
                          ...
                          placeholder="PIN"
                          autoFocus
                          className="w-24 px-3 py-2 text-center font-mono font-black text-base rounded-xl border-2 border-indigo-200 dark:border-indigo-600 focus:outline-indigo-500 bg-yellow-50/50 dark:bg-slate-900 text-slate-900 dark:text-white"
                        />
                        <button
                          type="submit"
                ...
                <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400 pt-1">
                  Tip: You can still click the "My Requests" tab above to check status on items you previously submitted.
                </div>
              </div>
            ) : (
