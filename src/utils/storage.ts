...
export const importDatabaseJSON = (jsonString: string): FamilyDatabase => {
  ...
  return {
    ...
    settings: {
      ...
      pauseRewardStore: parsed.settings?.pauseRewardStore ?? false,
      pauseRewardStoreReason: parsed.settings?.pauseRewardStoreReason || '',
      pauseSnackRequests: parsed.settings?.pauseSnackRequests ?? false,
      pauseSnackRequestsReason: parsed.settings?.pauseSnackRequestsReason || '',
      kioskTimeout: parsed.settings?.kioskTimeout || '5m',
      snackStarTiers: parsed.settings?.snackStarTiers,
